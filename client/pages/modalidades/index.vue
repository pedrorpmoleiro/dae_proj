      ...
      <v-card max-height="500px">
        <v-data-table hide-default-footer :items="escaloes" :headers="headersEscalao">
          ...
          <template v-slot:item.action="{ item }">
            ...
            <v-btn color="cyan" :to="`/modalidades/${epocaSelected.nome}/${modalidadeSelected}/${item.name}/horario`">
              Horario
            </v-btn>
          </template>
        </v-data-table>
      ...
      <v-col>
        <v-data-table :headers="headersE" item-key="nome" v-model="selected" :items="epocas"
          ...
          <template v-slot:item.data-table-select="{isSelected, select,item}">
            <v-simple-checkbox color="green" :value="isSelected" @input="select($event)"/>
          </template>
